...
class SelfTestScreen extends StatefulWidget {
  ...
  static Route<dynamic> route(RouteSettings routeSettings) {
    return CupertinoPageRoute(builder: (_) => const SelfTestScreen());
  }
}
...
class _SelfTestScreenState extends State<SelfTestScreen> {
  ...
  //id to pass for SelfTestQuestionsScreen
  ...
  late final _quizType =
      UiUtils.getCategoryTypeNumberFromQuizType(QuizTypes.SelfTest);
  late final _subType = UiUtils.subTypeFromQuizType(QuizTypes.SelfTest);
    ...
    if (context.read<SubCategoryCubit>().state is SubCategoryFetchFailure) {
      ...
      if ((context.read<SubCategoryCubit>().state as SubCategoryFetchFailure)
        ...
        if (selectedCategory != _defaultSelectedCategoryValue &&
          ...
          //to see what keys to pass in arguments see static function route of SelfTestQuesitonsScreen

          Navigator.of(context).pushNamed(
            Routes.SelfTestQuestions,
            arguments: {
  ...
  Widget build(BuildContext context) {
    ...
    final maxMinutes = config.SelfTestMaxMinutes < 3
        ? 1
        : config.SelfTestMaxMinutes ~/ 3;
    final maxQuestions = config.SelfTestMaxQuestions < 5
        ? 1
        : config.SelfTestMaxQuestions ~/ 5;
                    ...
                    SizedBox(
                      ...
                      child: ListView(
                        ...
                        children: List.generate(
                          maxQuestions,
                          (index) => config.SelfTestMaxQuestions < 5
                              ? config.SelfTestMaxQuestions
                              : (index + 1) * 5,
                        )
                            .map(
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: ListView(
                        ...
                        children: List.generate(
                          maxMinutes,
                          (index) => config.SelfTestMaxMinutes < 3
                              ? config.SelfTestMaxMinutes
                              : (index + 1) * 3,
                        )
                            .map(
                              (e) => _buildSelectTimeAndQuestionContainer(
