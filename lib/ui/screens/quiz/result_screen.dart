...
class _ResultScreenState extends State<ResultScreen> {
  ...
  late bool _isWinner;
  int _earnedCoins = 0;
  late int _earnedScore = widget.myPoints ?? 0;
  String? _winnerId;
    ...
    if (widget.quizType == QuizTypes.oneVsOneBattle) {
      ...
      if (widget.isPremiumCategory) {
        setState(() {
          ...
          _earnedScore = widget.myPoints! * 2;
        });
      }
    }
  ...
  Future<void> fetchUpdateUserDetails() async {
    if (widget.quizType == QuizTypes.quizZone ||
        ...
        widget.quizType == QuizTypes.listenandAnswer ||
        widget.quizType == QuizTypes.mathZone) {
      await context.read<UserDetailsCubit>().fetchUserDetails();
    }
  }

  void _updateStatistics() {
    if (widget.quizType != QuizTypes.SelfTest &&
  ...
  void _updateScoreAndCoinsDetails() {
    ...
    if (_isWinner) {
      ...
      } else if (widget.quizType == QuizTypes.listenandAnswer &&
          !widget.isPlayed) {
        _updateCoinsAndScore();
        context.read<SetCategoryPlayed>().setCategoryPlayed(
      ...
      } else if (widget.quizType == QuizTypes.mathZone && !widget.isPlayed) {
        _updateCoinsAndScore();
        context.read<SetCategoryPlayed>().setCategoryPlayed(
  ...
  void onPageBackCalls() {
    ...
    } else if (widget.quizType == QuizTypes.mathZone &&
        _isWinner &&
        !widget.isPlayed) {
      if (widget.questions!.first.subcategoryId == '0') {
        //update category
  ...
  bool showCoinsAndScore() {
    ...
    if (widget.quizType == QuizTypes.listenandAnswer) {
      //if user completed more than 30% and has not played this paragraph yet
      return _isWinner && !widget.isPlayed;
    }
    if (widget.quizType == QuizTypes.mathZone) {
      //if user completed more than 30% and has not played this paragraph yet
      return _isWinner && !widget.isPlayed;
    }
    return _isWinner;
    ...
    return Column(
      children: [
        ...
        if (widget.quizType == QuizTypes.quizZone ||
            ...
            widget.quizType == QuizTypes.SelfTest ||
            widget.quizType == QuizTypes.listenandAnswer ||
            widget.quizType == QuizTypes.guessTheWord ||
            widget.quizType == QuizTypes.funAndLearn ||
            widget.quizType == QuizTypes.mathZone) ...[
  ...
  String get _appbarTitle {
    final title = switch (widget.quizType) {
      QuizTypes.SelfTest => 'SelfTestResult',
      QuizTypes.listenandAnswer => 'audioQuizResult',
      QuizTypes.mathZone => 'mathQuizResult',
