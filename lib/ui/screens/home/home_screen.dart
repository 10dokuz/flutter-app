  ...
  static Route<HomeScreen> route(RouteSettings routeSettings) {
    return CupertinoPageRoute(
      builder: (_) => MultiBlocProvider(
        ...
        child: HomeScreen(
            isGuest: routeSettings.arguments == null
                ? true
...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  final battleZones = <ZoneType>[
    (title: 'groupPlay', img: Assets.TeamBattleIcon, desc: 'desgroupPlay'),
    (title: 'Battle Quiz', img: Assets.oneVsOneIcon, desc: 'desbattleQuiz'),
  ];
  ...
  final examZones = <ZoneType>[
    ...
    (
      title: 'Self-Test',
      img: Assets.SelfTestIcon,
      desc: 'challengeYourselfLbl'
    ),
  ];

  final playDifferentZones = <ZoneType>[
    (title: 'dailyQuiz', img: Assets.dailyQuizIcon, desc: 'desDdailyQuiz'),
    (
      title: 'funAndLearn',
    ...
    (title: 'listen and Answer', img: Assets.audioQuizIcon, desc: 'deslistenandAnswer'),
    (title: 'math Zone', img: Assets.mathsQuizIcon, desc: 'desmathZone'),
  ...
  void setQuizMenu() {
    Future.delayed(Duration.zero, () {
      ...
      if (!_sysConfigCubit.isAudioQuizEnabled) {
        playDifferentZones.removeWhere((e) => e.title == 'listen and Answer');
      }

      if (!_sysConfigCubit.isMathQuizEnabled) {
        playDifferentZones.removeWhere((e) => e.title == 'math Zone');
      }
      ...
      if (!_sysConfigCubit.isSelfTestQuizEnabled) {
        examZones.removeWhere((e) => e.title == 'SelfTest');
      }

      if (!_sysConfigCubit.isTeamBattleEnabled) {
      ...
      if (!_sysConfigCubit.isOneVsOneBattleEnabled &&
        ...
        battleZones.removeWhere((e) => e.title == 'Battle Quiz');
      }
  ...
  QuizTypes _getQuizTypeFromCategory(String category) {
    return switch (category) {
      'audio-question-category' => QuizTypes.listenandAnswer,
    ...
    switch (index) {
      ...
      case 'listen and Answer':
        Navigator.of(context).pushNamed(
          Routes.category,
          arguments: {
            'quizType': QuizTypes.listenandAnswer,
          },
        );
        return;
      case 'math Zone':
  ...
  void _onPressedSelfExam(String index) {
    ...
    if (index == 'exam') {
      ...
      Navigator.of(context).pushNamed(Routes.SelfTest);
    }
  }
