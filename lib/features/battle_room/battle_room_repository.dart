...
class BattleRoomRepository {
  ...
  Future<List<DocumentSnapshot>> searchBattleRoom({
    ...
    try {
      ...
      if (index != -1) {
        await deleteBattleRoom(documents[index].id, isTeamBattle: false);
  ...
  Future<void> deleteUnusedBattleRoom(String userId) async {
    try {
      ...
      for (final element in rooms['TeamBattle']!) {
  ...
  Future<List<Question>> getQuestions({
    ...
    try {
      ...
      if (roomCreator) {
        ...
        await deleteBattleRoom(roomDocumentId, isTeamBattle: forMultiUser);
      }
      throw BattleRoomException(errorMessageCode: e.toString());
    }
  }
