...
class SystemConfigCubit extends Cubit<SystemConfigState> {
  ...
  bool get isCategoryEnabledForTeamBattle =>
  ...
  int get TeamBattleMinimumEntryFee =>
      systemConfigModel?.TeamBattleMinimumEntryFee ?? 0;
  ...
  bool get isTeamBattleEnabled => systemConfigModel?.TeamBattleMode ?? false;
  ...
  int get SelfTestMaxMinutes =>
      systemConfigModel?.SelfTestMaxMinutes ?? 0;

  int get SelfTestMaxQuestions =>
      systemConfigModel?.SelfTestMaxQuestions ?? 0;
  ...
  bool get isSelfTestQuizEnabled =>
      systemConfigModel?.SelfTestMode ?? false;
  ...
  RoomCodeCharType get TeamBattleRoomCodeCharType =>
      systemConfigModel?.TeamBattleRoomCodeCharType ??
      RoomCodeCharType.onlyNumbers;
  ...
  bool isLatexEnabled(QuizTypes type) {
    return switch (type) {
      ...
      QuizTypes.SelfTest when _isQuizZoneLatexEnabled => true,
  ...
  int quizTimer(QuizTypes type) {
    final m = systemConfigModel;
    return switch (type) {
          QuizTypes.groupPlay => m?.TeamBattleTimer,
          ...
          QuizTypes.mathZone => m?.mathsQuizTimer,
          QuizTypes.listenandAnswer => m?.audioTimer,
  ...
  int quizCorrectAnswerCreditScore(QuizTypes type) {
    ...
    return switch (type) {
          ...
          QuizTypes.mathZone => m?.mathsQuizCorrectAnswerCreditScore,
          QuizTypes.listenandAnswer => m?.audioQuizCorrectAnswerCreditScore,
  ...
  int quizWrongAnswerDeductScore(QuizTypes type) {
    ...
    return switch (type) {
          ...
          QuizTypes.mathZone => m?.mathsQuizWrongAnswerDeductScore,
          QuizTypes.listenandAnswer => m?.audioQuizWrongAnswerDeductScore,
