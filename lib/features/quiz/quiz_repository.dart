...
class QuizRepository {
  ...
  Future<List<Question>> getQuestions(
    ...
    try {
      ...
      } else if (quizType == QuizTypes.SelfTest) {
        result = await _quizRemoteDataSource.getQuestionsForSelfTest(
          languageId: languageId!,
      ...
      } else if (quizType == QuizTypes.listenandAnswer) {
        final type = categoryId!.isNotEmpty ? 'category' : 'subcategory';
        final id = type == 'category' ? categoryId : subcategoryId!;
        result =
            await _quizRemoteDataSource.getlistenandAnswer(type: type, id: id);
      } else if (quizType == QuizTypes.mathZone) {
        final type = categoryId!.isNotEmpty ? 'category' : 'subcategory';
        final id = type == 'category' ? categoryId : subcategoryId!;
        result =
